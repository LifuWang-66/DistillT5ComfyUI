...
class T5Base(torch.nn.Module):
    def __init__(self, config_dict, dtype, device, operations):
        ...
        self.final_projection = torch.nn.Sequential(
            torch.nn.Linear(config_dict["project_in_dim"], config_dict["project_out_dim"], bias=False), 
            torch.nn.ReLU(), 
            torch.nn.Dropout(0.1), 
            torch.nn.Linear(config_dict["project_out_dim"], config_dict["project_out_dim"], bias=False) 
        )
        self.dtype = dtype
        self.shared = operations.Embedding(config_dict["vocab_size"], model_dim, device=device, dtype=dtype)
